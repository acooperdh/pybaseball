...
@pytest.fixture
def coords():
    return pl.DataFrame({"x": [1.0, 2.0, -1.0], "y": [1.0, 0.0, 10.0]})
...
def test_transform_coordinates(coords):
    ...
    expected = pl.DataFrame({"x": [1.0, 3.0, -3.0], "y": [-1.0, 1.0, -19.0]})
    ...
    assert_series_equal(pl.Series(name="x", data=3 * [xc]), transformed_coords.x)
    assert_series_equal(pl.Series(name="y", data=3 * [yc]), -transformed_coords.y)
