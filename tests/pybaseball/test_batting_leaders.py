...
def _sample_processed_result(get_data_file_dataframe: Callable[[str], pl.DataFrame]) -> pl.DataFrame:
...
def test_batting_stats(response_get_monkeypatch: Callable, sample_html: str,
    ...
    pl.testing.assert_frame_equal(batting_stats_result, sample_processed_result, check_dtype=False)
