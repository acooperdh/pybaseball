...
def try_parse_dataframe(
    ...
    for column in string_columns:
        ...
        if str(first_value).endswith('%') or column.endswith('%') or \
            ...
            # Doing it this way as just applying pl.to_datetime on
            ...
            for date_regex, date_format in date_formats:
                if isinstance(first_value, str) and date_regex.match(first_value):
                    data_copy[column] = data_copy[column].apply(pl.to_datetime, errors='ignore', format=date_format)
...
def coalesce_nulls(data: pl.DataFrame, value: Union[str, int, float, datetime] = np.nan) -> pl.DataFrame:
...
def columns_except(data: pl.DataFrame, columns: List[str]) -> List[str]:
    return list(np.setdiff1d(data.columns, columns))


def convert_numeric(data: pl.DataFrame, numeric_columns: List[str]) -> pl.DataFrame:
...
def convert_percentages(data: pl.DataFrame, columns: List[str]) -> pl.DataFrame:
...
def compute_pa(bat_df: pl.DataFrame) -> pl.Series:
    """
...
def augment_lahman_batting(bat_df: pl.DataFrame) -> pl.DataFrame:
    """
...
def augment_lahman_pitching(stats_df: pl.DataFrame) -> pl.DataFrame:
    """
...
def aggregate_by_season(stats_df: pl.DataFrame) -> pl.DataFrame:
